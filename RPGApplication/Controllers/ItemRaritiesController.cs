...
namespace RPGApplication.Controllers
{
    public class ItemRaritiesController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "RarityId,Name")] ItemRarity itemRarity)
        {
            if (ModelState.IsValid)
            {
                ItemRarity itemRarityInDataBase = ItemRarityDAO.Get(itemRarity.ItemRarityId);

                itemRarityInDataBase.Name = itemRarity.Name;

                ItemRarityDAO.Update(itemRarityInDataBase);
